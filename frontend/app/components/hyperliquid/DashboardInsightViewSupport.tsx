...
import type { LargeOrderZoneItem, NewsArticle } from '@/lib/api'

export type InsightExchange = 'hyperliquid' | 'binance'
export type InsightPeriod = '5m' | '15m' | '1h'
export type InsightWindow = '4h'
export type InsightAiState = 'idle' | 'monitoring' | 'thinking' | 'ready' | 'error'
export type InsightSentiment = 'bullish' | 'bearish' | 'mixed'
export type InsightDriverImpact = 'high' | 'medium' | 'low'
...
export interface InsightSentimentBreakdown {
  technical: number
  flow: number
  news: number
}

export interface InsightTechnicalLevel {
  price: number
...
export function formatPercent(value?: number | null, digits: number = 2) {
  if (value === null || value === undefined || Number.isNaN(value)) return '-'
  return `${value >= 0 ? '+' : ''}${value.toFixed(digits)}%`
}

export function formatAbsoluteUsd(value?: number | null) {
  return formatCompactUsd(Math.abs(value || 0))
}

export function formatPrice(value?: number | null, digits: number = 0) {
  if (value === null || value === undefined || Number.isNaN(value)) return '-'
...
export function getSentimentTheme(sentiment: InsightSentiment) {
...
export function getImpactBadgeClass(impact: InsightDriverImpact) {
...
export function getBreakdownBarClass(value: number) {
...
export function renderSemanticText(text: string) {
  if (!text) return null
...
export function formatInsightTarget(
...
export function getNetFlowPresentation(
  t: (key: string, fallback?: string, options?: Record<string, unknown>) => string,
  value?: number | null,
  inflowKey?: string,
...
export function normalizeRatio(value?: number | null) {
...
export function getPeriodWindowMs(period: InsightPeriod) {
  if (period === '5m') return 5 * 60 * 1000
  if (period === '15m') return 15 * 60 * 1000
  return 60 * 60 * 1000
}

export function getBucketStart(time: number, bucketMs: number) {
  return Math.floor(time / bucketMs) * bucketMs
}

export function formatBucketLabel(start: number, end: number) {
...
export function formatReactionPercent(chart: InsightChartPoint[], eventTime: number) {
...
export function formatReactionAtOffset(chart: InsightChartPoint[], eventTime: number, offsetMs: number) {
  if (!chart.length) return null
...
export function FlowEventIcon({ direction }: { direction: 'up' | 'down' }) {
  return (
...
export function InsightRefreshBadge({
  active,
}: {
  active: boolean
}) {
...
export function parseStructuredInsight(raw: string): StructuredAiInsight | null {
