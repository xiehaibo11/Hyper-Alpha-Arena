...
HYPER_AI_TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "exchange": {"type": "string", "enum": ["hyperliquid", "binance", "okx", "oke"], "description": "Exchange (default: hyperliquid)"}
                },
                "required": ["symbol"]
            }
        }
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Update trigger configuration for a Prompt-based AI Trader. Realtime signal pools are the primary trigger; scheduled trigger is an optional fallback.",
            "parameters": {
                "type": "object",
                "properties": {
                    "trader_id": {"type": "integer", "description": "AI Trader ID"},
                    ...
                    "signal_pool_ids": {
                        "type": "array",
                        "items": {"type": "integer"},
                        "description": "Signal pool IDs to bind for realtime decision triggers. Use [] to clear bindings."
                    },
                    "scheduled_trigger_enabled": {"type": "boolean", "description": "Enable optional scheduled fallback. Default is false."},
                    "trigger_interval": {"type": "integer", "description": "Scheduled fallback interval in seconds"}
                },
                "required": ["trader_id"]
            }
        }
...
HYPER_AI_TOOLS = merge_tool_definitions(
    HYPER_AI_TOOLS,
    EXCHANGE_QUERY_TOOLS,
    EXTERNAL_TOOLS,
    SKILL_TOOLS,
...
def execute_save_prompt(
    ...
    try:
        ...
        db.rollback()
        logger.warning(f"[save_prompt] Validation failed: {e}")
        return json.dumps({"error": str(e), "validation_failed": True})
    except Exception as e:
        db.rollback()
...
def execute_update_trader_strategy(
    ...
    try:
        ...
        updates = {"account_id": trader_id}
        if signal_pool_ids is not None:
            updates["signal_pool_ids"] = signal_pool_ids
        if scheduled_trigger_enabled is not None:
            updates["scheduled_trigger_enabled"] = scheduled_trigger_enabled
        if trigger_interval is not None:
            updates["trigger_interval"] = trigger_interval
        if exchange is not None:
            updates["exchange"] = exchange

        strategy = upsert_strategy(db, **updates)
        ...
        return json.dumps({
            ...
            "signal_pool_ids": json.loads(strategy.signal_pool_ids) if strategy.signal_pool_ids else [],
